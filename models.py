...
class PetUNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.cat([TF.center_crop(skip_layer_4, (56, 56)), x], dim=0)
        x = F.relu(self.conv3x3_1024LayersTo512Layers(x))
        x = F.relu(self.conv3x3_512LayersTo512Layers(x))
        x = self.conv2x2_512LayersTo256Layers(x)
        logger.debug(x.size())
        x = torch.cat([TF.center_crop(skip_layer_3, (104, 104)), x], dim=0)
        logger.debug(x.size())
        x = self.conv3x3_512LayersTo256Layers(x)
        x = self.conv3x3_256LayersTo256Layers(x)
        logger.debug(x.size())
        x = self.conv2x2_256LayersTo128Layers(x)
        x = torch.cat([TF.center_crop(skip_layer_2, (200, 200)), x], dim=0)
        logger.debug(x.size())
        x = self.conv3x3_256LayersTo128Layers(x)
        x = self.conv3x3_128LayersTo128Layers(x)
        logger.debug(x.size())
        x = self.conv2x2_128LayersTo64Layers(x)
        x = torch.cat([TF.center_crop(skip_layer_1, (392, 392)), x], dim=0)
        x = self.conv3x3_128LayersTo64Layers(x)
        x = self.conv3x3_64LayersTo64Layers(x)
        logger.debug(x.size())
